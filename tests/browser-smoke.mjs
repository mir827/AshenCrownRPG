...
const visualAudit = await desktop.evaluate(() => window.__ashenCrownDebug.getVisualAudit());
assert.equal(visualAudit.actors.length, 7);
assert.ok(visualAudit.actors.every(actor => actor.faceParts >= 6), `missing face features: ${JSON.stringify(visualAudit)}`);
assert.ok(visualAudit.actors.every(actor => actor.factionMarkers >= 3), `missing faction markers: ${JSON.stringify(visualAudit)}`);
const friendlyRings = new Set(visualAudit.actors.filter(actor => ['hero', 'ally', 'villager'].includes(actor.faction)).map(actor => actor.ringColor));
const hostileRings = new Set(visualAudit.actors.filter(actor => ['enemy', 'boss'].includes(actor.faction)).map(actor => actor.ringColor));
assert.equal([...friendlyRings].some(color => hostileRings.has(color)), false, 'friendly and hostile faction ring colors should remain distinct');
const vfxTuning = await desktop.evaluate(() => window.__ashenCrownDebug.getVfxTuning());
assert.ok(vfxTuning.basicSlashOuter <= 1.7 && vfxTuning.basicSparkCount <= 6, `basic slash VFX is too large/noisy: ${JSON.stringify(vfxTuning)}`);
assert.ok(vfxTuning.skillSlashOuter <= 3.25 && vfxTuning.skillSparkCount <= 10 && vfxTuning.skillFlashIntensity <= 36, `skill VFX is too large/noisy: ${JSON.stringify(vfxTuning)}`);
await desktop.locator('#begin').click();
const audioAfterBegin = await desktop.evaluate(() => window.__ashenCrownDebug.getAudioState());
assert.equal(audioAfterBegin.supported, true);
assert.equal(audioAfterBegin.started, true);
assert.equal(audioAfterBegin.muted, false);
assert.ok(audioAfterBegin.eventCounts['bgm-start'] >= 1 && audioAfterBegin.eventCounts.dialogue >= 1, `procedural BGM/dialogue audio did not start: ${JSON.stringify(audioAfterBegin)}`);
await desktop.locator('#audioToggle').click();
const mutedAudio = await desktop.evaluate(() => window.__ashenCrownDebug.getAudioState());
assert.equal(mutedAudio.muted, true);
assert.equal(await desktop.locator('#audioToggle').getAttribute('aria-pressed'), 'true');
await desktop.locator('#audioToggle').click();
const unmutedAudio = await desktop.evaluate(() => window.__ashenCrownDebug.getAudioState());
assert.equal(unmutedAudio.muted, false);
assert.ok(unmutedAudio.eventCounts.mute >= 1 && unmutedAudio.eventCounts.unmute >= 1, `mute control events missing: ${JSON.stringify(unmutedAudio)}`);
...
const audioAfterAttack = await desktop.evaluate(() => window.__ashenCrownDebug.getAudioState());
assert.ok(audioAfterAttack.eventCounts.attack >= 1, `attack SFX event missing: ${JSON.stringify(audioAfterAttack)}`);
...
const mobileAudio = await mobile.evaluate(() => window.__ashenCrownDebug.getAudioState());
assert.ok(mobileAudio.eventCounts.attack >= 1 && mobileAudio.eventCounts.skill >= 1 && mobileAudio.eventCounts.heal >= 1 && mobileAudio.eventCounts.dialogue >= 1, `mobile SFX events missing: ${JSON.stringify(mobileAudio)}`);
...
await mobile.close();
await browser.close();
console.log('PASS: desktop WebGL, reduced VFX tuning, face/faction audit, procedural audio/mute controls, mobile joystick, camera drag, pinch/button zoom, action buttons, layout and zero page errors');
