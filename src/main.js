...
const hud=document.createElement('div');hud.className='hud';hud.innerHTML=`<div class="topbar"><div class="name">카일 로언 · LV <b id="lv">1</b></div><div class="bar"><i id="hp"></i></div><div class="stats"><span id="hpText"></span> · 공격력 <b id="atk"></b> · 회복약 <b id="pots"></b></div></div><div class="quest"><small>QUEST LOG</small><div id="questText"></div></div><div id="bossHud" class="boss" hidden><b>심연 구동병기 · 모르가드</b><div class="bar"><i id="bossHp"></i></div></div><div class="skills"><div class="skill"><b>SPACE</b><span>서약 베기</span></div><div class="skill"><b>Q</b><span id="skillLabel">성흔 폭발</span></div><div class="skill"><b>R</b><span>회복약</span></div></div><div class="help">WASD 이동 · SHIFT 달리기 · 우클릭 드래그 시점 · 휠 줌 · E 상호작용</div><div id="prompt"></div><div id="layer"></div>`;root.append(hud);
...
function say(name,lines,onDone){let i=0;game.dialogue=true;const show=()=>{$('layer').innerHTML=`<div class="dialogue"><h3>${name}</h3><p>${lines[i]}</p><em>E 또는 클릭하여 계속</em></div>`};const next=()=>{i++;if(i>=lines.length){$('layer').innerHTML='';game.dialogue=null;removeEventListener('keydown',key);onDone?.()}else show()};const key=e=>{if(e.code==='KeyE'||e.code==='Space')next()};addEventListener('keydown',key);show();setTimeout(()=>document.querySelector('.dialogue')?.addEventListener('click',next),0)}
...
const keys={};addEventListener('keydown',e=>{keys[e.code]=true;if(!game.started||game.dialogue||game.ended)return;if(e.code==='KeyE')interact();if(e.code==='Space')attack(false);if(e.code==='KeyQ')attack(true);if(e.code==='KeyR'&&state.potions>0&&game.hp<game.maxHp){state.potions--;game.hp=Math.min(game.maxHp,game.hp+55);toast('회복약 사용 · HP +55')}});addEventListener('keyup',e=>keys[e.code]=false);
let yaw=0,pitch=.42,distance=10,drag=false,lx=0,ly=0;renderer.domElement.addEventListener('contextmenu',e=>e.preventDefault());renderer.domElement.addEventListener('pointerdown',e=>{if(e.button===2){drag=true;lx=e.clientX;ly=e.clientY}});addEventListener('pointerup',()=>drag=false);addEventListener('pointermove',e=>{if(drag){yaw-=(e.clientX-lx)*.006;pitch=THREE.MathUtils.clamp(pitch+(e.clientY-ly)*.004,.15,.9);lx=e.clientX;ly=e.clientY}});renderer.domElement.addEventListener('wheel',e=>distance=THREE.MathUtils.clamp(distance+e.deltaY*.01,6,16));
